...
def derive_keypair(
    seed: Union[str, bytes], index: int, curve: str = "ed25519"
) -> (hex, hex):
    """
...
def generate_deterministic_keypair(
    private_key, curve: str, origin_id: int
) -> (bytes, bytes):
    """
...
def derive_address(
    seed: Union[str, bytes], index: int, curve: str = "ed25519", algo: str = "sha256"
) -> hex:
    """
    Derive an address from a seed, an index, an elliptic curve and an hash algorithm.
    ...
    return (bytearray([curve_id]) + hashed_public_key).hex()
...
def ec_decrypt(
    ciphertext: Union[str, hex], private_key: Union[str, bytes], curve: str = "ed25519"
) -> any:
    """
    ...
    if curve_buf == 0:
        ...
        shared_key = bindings.crypto_scalarmult(
            curve_buf_pv, bytes(ephemeral_public_key)
        )
...
def aes_auth_decrypt(
    encrypted: bytes, aes_key: bytes, iv: bytes, tag: bytes
) -> (bytes, bytes):
    cipher = AES.new(aes_key, AES.MODE_GCM, iv)
