...
Based on https://github.com/lvzon/libsodium-python-examples
'''
# pylint: disable=C0103
# Import libnacl libs
from libnacl.version import version
# Import python libs
import ctypes
import sys
import os
...
crypto_scalarmult_ed25519_SCALARBYTES = nacl.crypto_scalarmult_ed25519_scalarbytes()
...
def crypto_scalarmult_ed25519(sk: bytes, pk: bytes):

    '''
    Generates shared key from pk and sk
    :param sk: signin key (bytes)
    :param pk: public key (bytes)
    :return: shared key (bytes)
    '''
    q = ctypes.create_string_buffer(crypto_scalarmult_ed25519_SCALARBYTES)
    ret = nacl.crypto_scalarmult(q, sk, pk)
    if ret:
        raise ValueError('Failed to generate keypair')
